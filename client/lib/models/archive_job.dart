...
class ArchiveJob {
  ...
  factory ArchiveJob.fromDatabase(Map<String, Object?> row) {
    final sourcePlatform = row['source_platform'] as String? ?? 'instagram';
    return ArchiveJob(
      id: row['id']! as String,
      sourcePostId:
          (row['source_post_id'] as String?) ??
          (sourcePlatform == 'xiaohongshu' ? '等待解析' : ''),
  ...
  String get failureTitle => switch (errorCode) {
    ...
    'XIAOHONGSHU_ERROR' ||
    'INVALID_RESPONSE' ||
    'SOURCE_MISMATCH' => '帖子解析失败',
    'VERIFICATION_REQUIRED' => '小红书需要验证',
  ...
  String get failureDetail {
    ...
    return switch (errorCode) {
      ...
      'XIAOHONGSHU_ERROR' ||
      'INVALID_RESPONSE' ||
      'SOURCE_MISMATCH' => '平台返回的帖子数据无法解析，请稍后重试。',
      'NETWORK_ERROR' => '无法连接内容平台，请检查系统网络或 VPN。',
      'RATE_LIMITED' => '平台请求过于频繁，请稍后重试。',
      'VERIFICATION_REQUIRED' => '小红书要求验证，当前匿名模式无法访问。',
