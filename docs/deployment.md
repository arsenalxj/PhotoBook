...
- 手动粘贴有效 Cookie 后显示真实用户名，输入框立即清空，杀进程重启后 Session 仍可使用；无效、过期、断网和限流分流正确，且失败不覆盖旧 Session。
- 登录成功或取消后，WebView Cookie、缓存和本地存储已清理；SQLite、R2、日志、通知和异常文本中没有 Cookie。只在可用会话下显示“复制Cookie”，复制已保存 Cookie 后 Flutter 不得收到其内容，系统剪贴板需标记敏感，进程存活时 60 秒后清理未被替换的 Cookie。
- 已登录账号可见的私密帖子仍返回不可访问。
- 分享后立刻回桌面、锁屏，再确认任务继续。
- 任务完成后前台服务通知消失。
